...
describe('syncMissedDemands()', () => {
  test('saves new demands found after lastActive and returns count', async () => {
    ...
    mockGetChatById.mockResolvedValue({ fetchMessages: jest.fn().mockResolvedValue([msg]) });
  ...
  test('skips messages older than lastActive', async () => {
    ...
    mockGetChatById.mockResolvedValue({ fetchMessages: jest.fn().mockResolvedValue([old]) });
  ...
  test('skips messages sent by the bot itself', async () => {
    ...
    mockGetChatById.mockResolvedValue({ fetchMessages: jest.fn().mockResolvedValue([mine]) });
  ...
  test('skips non-chat message types', async () => {
    ...
    mockGetChatById.mockResolvedValue({ fetchMessages: jest.fn().mockResolvedValue([audio]) });
  ...
  test('does not send summary message when count is 0', async () => {
    mockGetChatById.mockResolvedValue({ fetchMessages: jest.fn().mockResolvedValue([]) });
  ...
  test('sends plural message for multiple demands', async () => {
    ...
    mockGetChatById.mockResolvedValue({ fetchMessages: jest.fn().mockResolvedValue(msgs) });
